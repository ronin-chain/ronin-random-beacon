...
import (
	...
	randomRequestDB "github.com/ronin-chain/ronin-random-beacon/pkg/db/randomrequest"
...
func (l *RandomRequestListener) Process(ctx context.Context) {
	l.syncBatchLogs(ctx)
}
...
// Stores events to the database. Skip event if it is already stored.
func (l *RandomRequestListener) storeEvents(events []*contract.RandomBeaconCoordinatorRandomSeedRequested, ctx context.Context) error {
	...
	for _, event := range events {
		...
		eventPeriod := event.Period.Uint64()
		// Bypass if event is emitted.
		_, err = l.database.RandomRequest.Query().Where(randomRequestDB.Period(eventPeriod)).Only(context.Background())

		if err == nil {
			log.Info("[RandomRequestListener][storeEvents] event already exists", "period", eventPeriod)
			continue
		}
...
func (l *RandomRequestListener) syncBatchLogs(ctx context.Context) error {
