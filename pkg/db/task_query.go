...
type TaskQuery struct {
	...
	withRandomrequest *RandomRequestQuery
...
// QueryRandomrequest chains the current query on the "randomrequest" edge.
func (tq *TaskQuery) QueryRandomrequest() *RandomRequestQuery {
	query := (&RandomRequestClient{config: tq.config}).Query()
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withRandomrequest: tq.withRandomrequest.Clone(),
...
// WithRandomrequest tells the query-builder to eager-load the nodes that are connected to
// the "randomrequest" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithRandomrequest(opts ...func(*RandomRequestQuery)) *TaskQuery {
	query := (&RandomRequestClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withRandomrequest = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withRandomrequest; query != nil {
		if err := tq.loadRandomrequest(ctx, query, nodes,
			func(n *Task) { n.Edges.Randomrequest = []*RandomRequest{} },
			func(n *Task, e *RandomRequest) { n.Edges.Randomrequest = append(n.Edges.Randomrequest, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadRandomrequest(ctx context.Context, query *RandomRequestQuery, nodes []*Task, init func(*Task), assign func(*Task, *RandomRequest)) error {
	...
	query.withFKs = true
	query.Where(predicate.RandomRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.RandomrequestColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.task_randomrequest
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_randomrequest" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_randomrequest" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
