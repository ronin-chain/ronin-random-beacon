...
type ChainInfo struct {
	...
	// FinalizedBlock holds the value of the "finalized_block" field.
...
func (*ChainInfo) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case chaininfo.FieldProcessedBlock, chaininfo.FieldFinalizedBlock:
...
func (ci *ChainInfo) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case chaininfo.FieldFinalizedBlock:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field finalized_block", values[i])
			} else if value.Valid {
				ci.FinalizedBlock = uint64(value.Int64)
			}
		default:
...
func (ci *ChainInfo) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("finalized_block=")
	builder.WriteString(fmt.Sprintf("%v", ci.FinalizedBlock))
	builder.WriteByte(')')
