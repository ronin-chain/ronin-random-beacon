...
func (w *TaskCreator) createTasks() error {
	...
	for i, event := range events {
		...
		// Gen a Proof based on tracked event Request Including ( Period, PrevBeacon, ChainId, VerifyingContract, KeyHash)
		res, err := core.GenerateProofResponse(
			w.config.VRFConfig.Key,
			trackedEvent.Period,
			trackedEvent.Req.PrevBeacon,
			trackedEvent.Req.ChainId,
			trackedEvent.Req.VerifyingContract,
		...
		// Create struct FulFillRandomSeed
