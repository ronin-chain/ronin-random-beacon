...
type AppConfig struct {
	...
	ProcessedBlock           uint64         `mapstructure:"processed_block"`
...
func LoadConfig(configFilePath string) *AppConfig {
	...
	log.Info("Loaded Random Beacon Config", "CoordinatorContractAddress", config.CoordinatorAddress, "OracleAddress", config.OracleAddress, "NodeUrl", config.RpcEndpoint, "PublicKeyHash", config.VRFConfig.KeyHash)
	return config
}
